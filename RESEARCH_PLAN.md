...
- 문서 상태: BGV와 CGGE 인코더·디코더, deterministic source/digest-group split, hashlib verifier와 fixed-budget evaluator가 구현되었다. CGGE는 repository-fixed public-domain 8x8 glyph table과 SHA-256 checksum을 사용한다. source-prior random baseline 및 BGV/CGGE reversible·shuffled·zero controls는 구현되었고, Direct Bits 및 conditional model·학습 파이프라인은 미구현이다.
