...
def _write_cube_id_encoded_png(path: Path, payload: bytes) -> None:
...
def test_decode_sample_image_supports_cube_id_direct_bytes(tmp_path: Path) -> None:
    image_path = tmp_path / "source.png"
    _write_cube_id_encoded_png(image_path, b"\x12\x34")

    decoded = decode_sample_image(image_path, _byte2rgb_decoder("cube-id"))

    assert decoded["supported"] is True
    assert decoded["complete"] is True
    assert decoded["encoding"] == "rgb-cube-id"
    assert decoded["pixel_count"] == 2
    assert decoded["decoded_byte_count"] == 2
    assert decoded["ecc"]["code"] == "none"
    assert decoded["hex"] == "0x1234"
    assert decoded["rgb_colors"][0]["decoded_byte"] == 0x12
    assert decoded["rgb_colors"][0]["decoded_byte_hex"] == "0x12"
    assert decoded["rgb_colors"][0]["decoded_rgb_space"] is None
    assert decoded["rgb_colors"][0]["decoded_bit"] is None
...
def test_write_decode_comparison_uses_cube_id_for_cube_id_grid_fit_mode(
    tmp_path: Path,
) -> None:
    source_path = tmp_path / "source.png"
    final_path = tmp_path / "final.png"
    output_path = tmp_path / "decode_comparison.json"
    _write_cube_id_encoded_png(source_path, b"\x12\x34")
    _write_cube_id_encoded_png(final_path, b"\x12\x35")
    ...
    assert record["source"]["hex"] == "0x1234"
    assert record["final"]["hex"] == "0x1235"
    assert record["source"]["encoding"] == "rgb-cube-id"
    assert record["final"]["encoding"] == "rgb-cube-id"
